...
/// Fix 2: inset = inputBarHeight only (no safe area)
/// Theory: safe area is BELOW input bar, not between content and input bar
struct ChatFix2_InputBarHeightOnly: View {
    let conversation: Conversation

    var body: some View {
        Fix2ViewController_Wrapper(conversation: conversation)
            .ignoresSafeArea()
            .navigationTitle("Fix 2: inputBarHeight only")
            .navigationBarTitleDisplayMode(.inline)
    }
}

private struct Fix2ViewController_Wrapper: UIViewControllerRepresentable {
    let conversation: Conversation

    func makeUIViewController(context: Context) -> Fix2ViewController {
        Fix2ViewController(messages: conversation.messages)
    }

    func updateUIViewController(_ uiViewController: Fix2ViewController, context: Context) {}
}

private final class Fix2ViewController: UIViewController {
    ...
    @objc private func keyboardWillChange(_ notification: Notification) {
        ...
        let newBottomInset = inputBarHeight + effectiveKeyboardHeight
    ...
    private func updateScrollViewInsets() {
        ...
        let newBottomInset = inputBarHeight
