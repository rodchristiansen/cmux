...
/// Fix 5: Reduce bottom padding
/// Remove the 8pt bottom padding on content stack
struct ChatFix5_ReducePadding: View {
    let conversation: Conversation

    var body: some View {
        Fix5ViewController_Wrapper(conversation: conversation)
            .ignoresSafeArea()
            .navigationTitle("Fix 5: no bottom padding")
            .navigationBarTitleDisplayMode(.inline)
    }
}

private struct Fix5ViewController_Wrapper: UIViewControllerRepresentable {
    let conversation: Conversation

    func makeUIViewController(context: Context) -> Fix5ViewController {
        Fix5ViewController(messages: conversation.messages)
    }

    func updateUIViewController(_ uiViewController: Fix5ViewController, context: Context) {}
}

private final class Fix5ViewController: UIViewController {
    ...
    private func setupConstraints() {
        ...
        NSLayoutConstraint.activate([
            ...
            // Fix 5: no bottom padding (0 instead of -8)
            contentStack.bottomAnchor.constraint(equalTo: scrollView.contentLayoutGuide.bottomAnchor, constant: 0),
