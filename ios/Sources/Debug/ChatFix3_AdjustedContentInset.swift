...
/// Fix 3: Use adjustedContentInset
/// Check what iOS actually applies, then set our inset to complement it
struct ChatFix3_AdjustedContentInset: View {
    let conversation: Conversation

    var body: some View {
        Fix3ViewController_Wrapper(conversation: conversation)
            .ignoresSafeArea()
            .navigationTitle("Fix 3: adjustedContentInset")
            .navigationBarTitleDisplayMode(.inline)
    }
}

private struct Fix3ViewController_Wrapper: UIViewControllerRepresentable {
    let conversation: Conversation

    func makeUIViewController(context: Context) -> Fix3ViewController {
        Fix3ViewController(messages: conversation.messages)
    }

    func updateUIViewController(_ uiViewController: Fix3ViewController, context: Context) {}
}

private final class Fix3ViewController: UIViewController {
