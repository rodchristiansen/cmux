...
def _wait_focused_index(client: cmux, index: int, timeout: float = 2.0) -> bool:
    start = time.time()
...
def test_close_middle_keeps_index(client: cmux) -> TestResult:
    ...
    try:
        ...
        if not _wait_focused_index(client, 1, timeout=2.0):
...
def test_close_last_selects_previous(client: cmux) -> TestResult:
    ...
    try:
        ...
        if not _wait_focused_index(client, last_index, timeout=2.0):
