#!/usr/bin/env python3
"""
Regression test: `cmux claude-teams` keeps main-vertical anchored to the leader pane.
"""
...
LEADER_STACK_PANE_ID = "77777777-7777-4777-8777-777777777777"
LEADER_STACK_SURFACE_ID = "88888888-8888-4888-8888-888888888888"
NOISE_PANE_ID = "99999999-9999-4999-8999-999999999999"
NOISE_SURFACE_ID = "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"
TEAM_TWO_PANE_ID = "bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb"
TEAM_TWO_SURFACE_ID = "cccccccc-cccc-4ccc-8ccc-cccccccccccc"
...
class FakeCmuxState:
    def __init__(self) -> None:
        ...
        self.pending_splits = [
            (LEADER_STACK_PANE_ID, LEADER_STACK_SURFACE_ID, "pane:3", "surface:3", "leader-stack"),
            (NOISE_PANE_ID, NOISE_SURFACE_ID, "pane:4", "surface:4", "noise"),
            (TEAM_TWO_PANE_ID, TEAM_TWO_SURFACE_ID, "pane:5", "surface:5", "teammate-2"),
        ]
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="ct-column-", dir="/tmp") as td:
        tmp = Path(td)
        home = tmp / "home"
        ...
        noise_split_log = tmp / "noise-split.log"
...
noise_split="$(tmux split-window -t "%{TOP_PANE_ID}" -h -P -F '#{{pane_id}}')"
...
printf '%s\\n' "$noise_split" > "$FAKE_NOISE_SPLIT_LOG"
...
""",
        ...
        env["FAKE_NOISE_SPLIT_LOG"] = str(noise_split_log)
        ...
        if read_text(first_split_log) != f"%{LEADER_STACK_PANE_ID}":
            print(f"FAIL: expected first split to print %{LEADER_STACK_PANE_ID}, got {read_text(first_split_log)!r}")
            return 1

        if read_text(noise_split_log) != f"%{NOISE_PANE_ID}":
            print(f"FAIL: expected noise split to print %{NOISE_PANE_ID}, got {read_text(noise_split_log)!r}")
            return 1
        ...
        expected = [
            ...
            {
                "workspace_id": WORKSPACE_ID,
                "surface_id": LEADER_STACK_SURFACE_ID,
                "direction": "down",
            },
        ]
    ...
    print("PASS: cmux claude-teams keeps main-vertical anchored to the leader pane")
    return 0
