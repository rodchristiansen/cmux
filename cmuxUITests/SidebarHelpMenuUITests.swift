...
final class CommandPaletteAllSurfacesUITests: XCTestCase {
    ...
    private func socketCommand(_ command: String) -> String? {
        UITestControlSocketClient(path: socketPath, responseTimeout: 2.0).sendLine(command)
    }
    ...
    private func socketJSON(method: String, params: [String: Any]) -> [String: Any]? {
        ...
        return UITestControlSocketClient(path: socketPath, responseTimeout: 2.0).sendJSON(request)
    }
}

// Shared unix-socket client used by UI tests that drive the control socket directly.
final class UITestControlSocketClient {
    private let path: String
    private let responseTimeout: TimeInterval?

    init(path: String, responseTimeout: TimeInterval? = nil) {
        self.path = path
